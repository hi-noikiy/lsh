...
public class Jurisdiction {
	...
	public static Integer getAppUserId(){
		Object id= getSession().getAttribute(Const.SESSION_APP_USER_ID);
		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
		if(id!=null){
			return (Integer)id;
		}else{
			id=request.getParameter("token");
			id=id==null?getCookie(request, "token"):id;
			if(id!=null&&!"".equals(id)){
				//普通类从spring容器中拿出service
				WebApplicationContext webctx=ContextLoader.getCurrentWebApplicationContext();
				LshUserService userService = (LshUserService)webctx.getBean("lshuserService");
				PageData pd=new PageData();
				pd.put("TOKEN", id);
				try {
					PageData user=userService.findById(pd);
					if(user!=null){
						return Integer.valueOf(user.get("USER_ID").toString());
					}
				} catch (Exception e) {
	 ...
	 * @return
	 */
	public static String getAppUserRole(){
		Object id= getSession().getAttribute(Const.SESSION_APP_USER_ROLE);
		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
		if(id!=null){
			return id.toString();
		}else{
