 ...
 */
@Controller
@RequestMapping(value="/wechatmedia")
public class WechatMediaController extends BaseController {
	
	String menuUrl = "wechatmedia/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save(MultipartFile pictureFile) throws Exception{
		...
		AttachUploadResult result=WechatInterfaceInvokeUtil.uploadAttach(path, false);//上传临时素材
		pd.put("TYPE", result.getType());
		pd.put("MEDIA_ID", result.getMedia_id());
		pd.put("CREATED_AT", result.getCreated_at());
		pd.put("LOCATION", path);
		pd.put("TEMPORARYORMATERIAL", "0");
	...
	public ModelAndView list(Page page) throws Exception{
		...
		mv.setViewName("system/wechat/wechatmedia/wechatmedia_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("system/wechat/wechatmedia/wechatmedia_edit");
