...
//		if("0".equals(carList.get(0).get("GOODS_POSITION").toString())){
//			PageData config = sysconfigService.findById(pd); // 当前系统参数
//			String postage = config.get("POSTAGE").toString();
//			pd.put("postage", postage);
//		}
		PageData config = sysconfigService.findById(pd); // 当前系统参数
		...
		if(content > Double.parseDouble(free_shipping_amount)){
			...
			pd.put("postage", config.get("POSTAGE").toString());
			pd.put("content", String.format("%.2f", content+Double.parseDouble(config.get("POSTAGE").toString()))); // 商品总价
	...
	public Object createOrder(HttpServletRequest request) throws Exception {
		...
		// 实付金额大于免邮金额
		if ((content - discount) > Double.parseDouble(config.get("FREE_SHIPPING_AMOUNT").toString())) {
		...
		// 修改抽奖记录参数
		PageData editAwardRecord = new PageData();
		editAwardRecord.put("ORDER_ID", orderId + "");
		editAwardRecord.put("CART_ID", pd.get("ids"));
		editAwardRecord.put("USER_ID", user_id);
		editAwardRecord.put("STEP", 2);
		awardrecordService.editOrder(editAwardRecord);
	...
	public String save_binding_alipay(HttpServletRequest request)
		...
		PageData editAwardRecord = new PageData();
		editAwardRecord.put("CART_ID", pd.get("CART_ID"));
		editAwardRecord.put("USER_ID", user_id);
		editAwardRecord.put("STEP", 1);
		awardrecordService.editOrder(editAwardRecord);
		return "success";
	}
	...
	@ResponseBody
	public String cancel_order(HttpServletRequest request,
	...
	@ResponseBody
	public String confirm_goods(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		PageData pd = new PageData();
		pd = this.getPageData();
		Integer user_id = Jurisdiction.getAppUserId();
