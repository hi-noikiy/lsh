package com.szjm.controller.lsh.agent;
...
public class AgentController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增Agent");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("AGENT_ID", this.get32UUID());	//主键
		pd.put("CREATE_USER", Jurisdiction.getUsername());	//创建人
		pd.put("CREATE_TIME", Tools.date2Str(new Date()));	//创建时间
		pd.put("UPDATE_USER", "");	//修改人
		pd.put("UPDATE_TIME", "");	//修改时间
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Agent");
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Agent");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("UPDATE_USER", Tools.date2Str(new Date()));	//修改人
		pd.put("UPDATE_TIME", Jurisdiction.getUsername());	//修改时间
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Agent");
		...
		mv.setViewName("lsh/agent/agent_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("lsh/agent/agent_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Agent");
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Agent到excel");
		...
		titles.add("内容");	//5
		titles.add("内容");	//6
		titles.add("状态（0.会员，1.经销商，3.企商VIP）");	//7
		titles.add("创建人");	//8
		titles.add("创建时间");	//9
		titles.add("修改人");	//10
		titles.add("修改时间");	//11
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var1", varOList.get(i).get("AGENT_ID").toString());	//1
			vpd.put("var2", varOList.get(i).get("PRICE").toString());	//2
			vpd.put("var3", varOList.get(i).getString("TITLE"));	    //3
			vpd.put("var4", varOList.get(i).getString("ITEM_ONE"));	    //4
			vpd.put("var5", varOList.get(i).getString("ITEM_TWO"));	    //5
			vpd.put("var6", varOList.get(i).getString("ITEM_THREE"));	    //6
			...
			vpd.put("var10", varOList.get(i).getString("UPDATE_USER"));	    //10
