...
(function() {
	...
	b.isPointInPolygon = function(o, l) {
		...
		for (var f = 1; f <= h; ++f) {
			...
			q = t[f % h];
			if (e.lat < Math.min(s.lat, q.lat)
					|| e.lat > Math.max(s.lat, q.lat)) {
				s = q;
				continue
			}
			if (e.lat > Math.min(s.lat, q.lat)
	...
	b.getDistance = function(j, h) {
		...
		return a
				* Math.acos((Math.sin(i) * Math.sin(g) + Math.cos(i)
						* Math.cos(g) * Math.cos(e - f)))
	};
	...
	b.getPolygonArea = function(t) {
		...
		for (var P = 0; P < B; P++) {
			...
			Q = N * v - p;
			m = (g * s + z * O + Q * l)
					/ (Math.sqrt(g * g + z * z + Q * Q) * Math.sqrt(s * s + O
							* O + l * l));
			m = Math.acos(m);
			G = z * l - Q * O;
			j = 0 - (g * l - Q * s);
