...
<body class="no-skin">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">发帖人</th>
									<th class="center">标题</th>
									<th class="center">帖子内容</th>
									<th class="center">浏览量</th>
									<th class="center">用户编号</th>
									<th class="center">收藏量</th>
									<th class="center">点赞量</th>
									
									<th class="center">发帖时间</th>
									<th class="center">APP展示状态</th>						
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.VISITOR_VOLUME}</td>
											<td class='center'>${var.USER_ID}</td>
											<td class='center'>${var.COLLECTION_VOLUME}</td>
											<td class='center'>${var.PRAISE_VOLUME}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="查看详情" onclick="articleDetail('${var.ARTICLE_ID}');">
														查看详情
													</a>
													<a class="btn btn-xs btn-success" title="查看评论" onclick="articleComment('${var.ARTICLE_ID}');">
														查看评论
													</a>
													<c:if test="${var.DELETE_STATUS == 0}">
														<a class="btn btn-xs btn-success" title="隐藏帖子" onclick="articleDel('${var.ARTICLE_ID}','1');">
															隐藏帖子
														</a>
													</c:if>
													<c:if test="${var.DELETE_STATUS == 1}">
														<a class="btn btn-xs btn-success" title="显示帖子" onclick="articleDel('${var.ARTICLE_ID}','0');">
															展示帖子
														</a>
													</c:if>
													</c:if>
	...
	<script type="text/javascript">
		...
		function articleDel(Id,status){
			top.jzts();
			var url = "<%=basePath%>article/articleDel.do?ARTICLE_ID="+Id+"&tm="+new Date().getTime();
 ...
 --%>   //显示隐藏帖子
		function articleDel(Id,status){
			 $.ajax({
				type: "POST",
				url: '<%=basePath%>article/articleDel.do',
		    	data: {ARTICLE_ID:Id,status:status},
				//dataType:'json',
				cache: false,
		...
		function articleDetail(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="帖子详情";
			 diag.URL = '<%=basePath%>article/goArticleDetail.do?ARTICLE_ID='+Id;
			 diag.Width = 500;
			 diag.Height = 555;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function articleComment(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="帖子评论";
			 diag.URL = '<%=basePath%>article/articleCommentList.do?ARTICLE_ID='+Id;
			 diag.Width = 1000;
			 diag.Height = 555;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
