...
<body style="background-color: white;">
    ...
    window.onload = function() {
	    ...
	    <c:if test="${y!=null&&y!=''}">/* alert(${y}+""+${m}+""+${d}); */
	    	globledate=new Date(${y},${m-1},${d});
	    </c:if>
    ...
    function bind_click(a) {
        ...
        $("#" + a).find("td").each(function() {
            if ($(this).hasClass('not_this')) {
                ...
                $(this).click(function() {
                    set_top($(this));
                    var by=$(this).attr('data_y');
                    var bm=$(this).attr('data_m');
                    var bd=$(this).attr('data_d');
                    setDateAndGoBack(by,bm,bd);
                });
            }
        });
    }
	function setDateAndGoBack(by,bm,bd){
        if(bm.length<2){
        	bm="0"+bm;
        };
        if(bd.length<2){
        ...
        var ymd=by+"-"+bm+"-"+bd;
        localStorage.val=ymd;
        var d=new Date();
        localStorage.time=d.getTime()+3500;
        //window.close();
        window.history.go(-1);
	}
