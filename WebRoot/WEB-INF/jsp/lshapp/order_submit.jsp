...
.navigat_image{
			...
			<div class="flow-checkout">
				...
				<div id="flow-cont-height" >
					...
					<section class="m-top10">
						<section class="flow-checkout-pro j-flow-checkout-pro  active">
							...
							<div class="product-list-small m-top1px b-color-f dis-box">
								...
								<c:forEach items="${chickList}" var="var" varStatus="vs">
									<li>
											...
											<div class="product-text">
												...
												<p>
													<span class="p-price t-first ">¥${var.GOODS_CURRENT_PRICE } <small
														class="fr t-remark">x${var.AMOUNT }</small></span>
												</p>
					...
					<!-- <ul>
							...
							<label class="t-remark g-t-temark">快递费信息</label>
							</li>
						</ul>
						<p class="padding-all f-05 text-right t-remark">
							<select style="width: 80px;">
								<option>包邮</option>
								<option>到付（约为10元）</option>
								<option>快递费20元</option>
							</select>
						</p>
					 -->
						...
						<p class="padding-all f-05 text-right t-remark">
							共计：${pd.goodsSize } 件商品，优惠：<span class="t-first" id="ru_amount0">¥${pd.discount }&nbsp;</span>快递费：<span class="t-first" id="ru_amount0">

							<c:if test="${pd.postage == 0 }">
								包邮
							</c:if>
							<c:if test="${pd.postage != 0 }">
								¥${pd.postage }
						...
						<p class="padding-all f-05 text-right t-remark">
							合计：<span class="t-first" id="ru_amount0">¥${pd.content }</span>
						</p>
	...
	<script type="text/javascript">
		function goAddressList(){ // 去地址列表
			window.location.href="<%=basePath%>lshapp/order/order_address.do?type=order";
		}
		...
		function submitOrder(){
			...
			if(address == null || address == ""){
			 ...
			 $.ajax({
	            type: "POST",
	            url: '<%=basePath%>lshapp/order/createOrder.do',
				data: {"ADDRESS_ID":ADDRESS_ID,"ids":ids,"payment":payment},
				dataType: "json",
