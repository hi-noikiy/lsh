...
<body>
					...
					<div class="show-goods-attr j-filter-show-div ts-3 b-color-1">
						<section class="s-g-attr-title b-color-1  product-list-small">
							<div class="product-div" style="overflow: inherit;">
								...
								<div class="product-text">
									...
									<div>
										<span class="f-06 t-first" style="display: none;">¥<span id="ECS_GOODS_AMOUNT">${goods.CURRENT_PRICE }</span></span>
										<span class="f-06 t-first" >¥<span >0</span></span>
									</div>
						...
						<section
							...
							<div class="swiper-wrapper">
								<div class="swiper-slide swiper-slide-active">
									...
									<h4 class="t-remark" style="display: none;">数量</h4>
									<!-- 普通商品可修改数量 -->
									<div class="div-num dis-box m-top08" style="display: none;">
						...
						<section class="ect-button-more dis-box">
							<div class="btn-submit box-flex add-to-cart"
								onclick="goExchangeSubmit('${pd.CART_ID}','${pd.GOODS_ID}')">确认</div>
						</section>
					</div>
					<!--商品属性弹出层end-->
				</section>
	...
	<script type="text/javascript">
		...
		//   领取奖品，先修改购物车参数，再提交付钱
		function goExchangeSubmit(CART_ID,GOODS_ID){
			if(skuId == ""){
				 d_messages("请选择商品规格");
			}else{
				 $.ajax({
                     type:"post",
                     url:"<%=basePath%>lshapp/order/update_cart.do",
                     data:{CART_ID:CART_ID,SKU_ID:skuId},
                     success:function(ret){
                    	 window.location.replace("<%=basePath%>lshapp/order/submit.do?ids="+CART_ID);
                     }
				})
