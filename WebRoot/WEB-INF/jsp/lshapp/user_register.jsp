...
<!--     </div> -->
<!--  </div> -->
	<input type="hidden" placeholder="请输入昵称" value="未知昵称" class="input"id="NICK_NAME" name="NICK_NAME">
  <div style="height: 50px;width: 100%;text-align: center;">
	<div style="float:left;height: 50px;width: 21%;line-height:50px">登录密码</div>
	<div class="text" style="width: 78%;float: right;">
...
<script type="text/javascript">
		...
		function save(){
				...
				$.ajax({
			        ...
			        success: function(data){// 请求失败处理函数
			        	if(data=="success"){
			        		$("#butts").attr("disabled","disabled");
			        		$.ajax({
						        type : 'POST',
						        url : "<%=basePath%>lshapp/appLogin/save.do",
						        data:{"INVITE_CODE":INVITE_CODE,"NICK_NAME":NICK_NAME,"PASSWORD":PASSWORD,"PHONE_NUMBER":PHONE_NUMBER,"OPENID":OPENID,"SEX":SEX},
						        success: function(data){// 请求失败处理函数
						        	if(data== "0"){
						        		d_messages("微信号已存在 ！");
						        		$("#butts").attr("disabled","");
