	...
	<script type="text/javascript">
		function chaxai(){
			...
			$.ajax({
     		 		async : false,
			        type : 'POST',
			        url : "<%=basePath%>lshapp/userCenter/go_integration_details.do?lastStart="+lastStart+"&lastEnd="+lastEnd+"&currentPage=1",// 请求的action路径
			        success: function(data){// 请求失败处理函数
			        	$("#integration").html("");
			        	$("#integration").html(data) //将ajxa请求的数据追加到内容里
			        },
			    });
		}
	</script>
              ...
              if($(document).scrollTop()>=$(document).height()-$(window).height()){
            	  ...
            	  if(parseInt(totalPage)<parseInt(currentPage)){ // 没有更多了
            		  ...
            		  if(!isMore){
		                ...
		                $.ajax({
		                    type:"post",
		                    url:"<%=basePath%>lshapp/userCenter/go_integration_details.do",
		                    ...
		                    success:function(ret){
		                   		$("#integration").after(ret) //将ajxa请求的数据追加到内容里
...
<div id="loading" style="display: none;"><img src="static/lshapp/images/center/loading.gif"></div>
		<!-- <section class="search category-search common-search" style="background-color: #ec5151;">
	       <div class="text-all dis-box j-text-all text-all-back">
					  ...
					  <div style="text-align: center;width: 86%;" >
	                        <span style="text-align: center;font-size: 1.8rem;color: white;">礼豆</span>
					  </div>
			</div>
		...
		<input type="hidden" name="totalPage" id="totalPage" value="${page.totalPage}"/>
			<p style="color: white;">今日收益(豆)</p>
			<h2 style="color: white;">${today}</h2>
			<h5 >总礼豆：${pd.INTEGRATION}</h5>
		...
		<section class="b-color-f my-nav-box m-top10">
			...
			<hr style="height:1px;border:none;border-top:1px solid #E1E1E1;width: 90%;margin-top: 0.3rem;margin-left: 5%;" />
			<div style="margin-top: -1.0rem;" id="integration">
