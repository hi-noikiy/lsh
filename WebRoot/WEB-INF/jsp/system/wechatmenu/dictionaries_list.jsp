...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="wechatMenu/list.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'><a href="javascript:goSondict('${var.DICTIONARIES_ID }')">${var.TYPE}</a></td>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<a class="btn btn-sm btn-success" onclick="setMenu();">应用菜单到公众号</a>
									<a class="btn btn-sm btn-success" onclick="deleteMenu();">删除公众号中的菜单</a>
								</td>
	...
	<script type="text/javascript">
		...
		function goSondict(DICTIONARIES_ID){
			top.jzts();
			window.location.href="<%=basePath%>wechatMenu/list.do?DICTIONARIES_ID="+DICTIONARIES_ID;
		};
		function setMenu(){
			bootbox.confirm("确定要设置菜单？", function(result) {
				if(result) {
					...
					$.ajax(
							{
								...
								url:'${pageContext.request.contextPath}/wechatMenu/setMenu.do',
								//ContentType没指定将默认为：application/x-www-form-urlencoded
								data:'',
								success:function(data){
									if(data.status=='1'){
										$(top.hangge());
										bootbox.confirm("菜单设置成功！", function(result) {
											if(result) {
											}else{
											}
										});
										...
										$(top.hangge());
										bootbox.confirm("菜单设置失败！("+data.message+")", function(result) {
											if(result) {
											}else{
											}
										});
		...
		function deleteMenu(){
			bootbox.confirm("确定要删除菜单？", function(result) {
				if(result) {
					top.jzts();
					$.ajax(
							{
								type:'post',//这里改为get也可以正常执行
								url:'${pageContext.request.contextPath}/wechatMenu/deleteMenu.do',
								//ContentType没指定将默认为：application/x-www-form-urlencoded
								data:'',
								success:function(data){
									if(data.status=='1'){
										$(top.hangge());
										bootbox.confirm("菜单删除成功！", function(result) {
											if(result) {
											}else{
											}
										});
										...
										$(top.hangge());
										bootbox.confirm("菜单删除失败！", function(result) {
											if(result) {
											}else{
											}
										});
		...
		function add(DICTIONARIES_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>wechatMenu/goAdd.do?DICTIONARIES_ID='+DICTIONARIES_ID;
			 diag.Width = 500;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if('none' == diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display){
					 parent.location.href="<%=basePath%>wechatMenu/listAllDict.do?DICTIONARIES_ID=${DICTIONARIES_ID}&dnowPage=${page.currentPage}";
				}
				diag.close();
			 };
			 diag.show();
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>wechatMenu/delete.do?DICTIONARIES_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>wechatMenu/goEdit.do?DICTIONARIES_ID='+Id;
			 diag.Width = 500;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
